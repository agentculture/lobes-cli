...
## Update 5 — PR #279 open (2026-09-18, late)

- **PR:** https://github.com/agentculture/lobes-cli/pull/279 — branch pushed, version **0.81.0**, suite **6002 passed / 15 skipped**, site 249/249. First CI: lint, secrets-scan, site-build, version-check, GitGuardian green; `test` was still running. Qodo review pending — triage per the PR workflow (reply to every thread, fix or push back, resolve).
- Merged since Update 4: t15 (`lobes init --fleet --audio --audio-lang he`, overlay in the compose chain, doctor heal, golden under `tests/goldens/overlays/`; merge semantics checked with a real `docker compose config`), d3 advert `language` (absent unless declared; `lobes.roles.role_payload` is THE serializer; byte-identical to `main` verified), docs (`docs/hebrew-realtime.md`, `docs/contracts/` + README, realtime-pipeline boundaries, explain catalog, CLAUDE.md), one-take-back-per-utterance fix, web harness fixes (jitter buffer, status line, blank bubbles).
- Web app: `astro dev` daemon on the Spark, 127.0.0.1:4321 (stop: `cd site && npx astro dev stop`). Driven in a real Chrome on the box. Operator has NOT yet confirmed the stutter fix by ear; browser tool-call path untested (needs a human voice — the reSpeaker cancels anything played through its own speaker).
- `chatterbox` container stopped (−7 GiB). Stack ≈ 47 GiB. `TTS_DEBUG_TEXT=1` still ON in `~/.lobes/.env` (logs spoken text) — turn off before household use.
- Issues: #276 (26B on Orin), #277 (mixed Hebrew/English), #278 (write the remaining contracts). Consumer instructions posted on agentculture/shabbos-goy#1 (recommends ears-only mode).
- **Still owed:** t17 acceptance transcript FROM THE PR's TestPyPI dev wheel (the live box runs a hand-carried local wheel named 0.80.1 — rebuild needs the override's `LOCAL_WHEEL` arg updated to 0.81.0, or the dev wheel), then `/validate-delivery`, `/summarize-delivery`; operator to adjudicate lapses l3, l4; blank-noise barge-in design question; BlueTTS licence (operator is asking the author); PocketTTS Hebrew A/B if the operator can share the checkpoint.
