# Implementation Split Plan — hebrew-realtime

slug: `hebrew-realtime` · generated by `assign-to-workforce.sh split-plan --write` from `devague plan waves --json` (issue #82). Re-running this command overwrites this file in place; edits made to the **Task assignments** table below survive a regeneration, matched by task id.

## Dependency waves

- Wave 1: `t1`, `t4`, `t9`, `t13`
- Wave 2: `t2`, `t3`, `t5`, `t6`, `t8`, `t14`
- Wave 3: `t7`, `t10`
- Wave 4: `t11`, `t12`
- Wave 5: `t15`
- Wave 6: `t16`
...
| Task | Owner | Model |
| --- | --- | --- |
| `t1` | main agent, in-house (live box) | fable |
| `t4` | worktree subagent | opus |
| `t9` | worktree subagent | sonnet |
| `t13` | worktree subagent | sonnet |
| `t2` | main agent, in-house (live box) | fable |
| `t3` | main agent + operator listening | fable |
| `t5` | worktree subagent | sonnet |
| `t6` | worktree subagent | sonnet |
| `t8` | worktree subagent | sonnet |
| `t14` | worktree subagent | sonnet |
| `t7` | worktree subagent | opus |
| `t10` | worktree subagent | sonnet |
| `t11` | worktree subagent | sonnet |
| `t12` | worktree subagent | opus |
| `t15` | worktree subagent | opus |
| `t16` | worktree subagent | sonnet |
| `t17` | main agent + operator speaking | fable |
...
- [unknown_nonblocking] Which runtime serves the ivrit.ai Whisper checkpoint on the GB10 (transformers+PyTorch vs vLLM's Whisper path) is unmeasured; t10's Dockerfile cannot be written until t2 picks one
- [unknown_nonblocking] Which Hebrew TTS engine wins is unmeasured and depends on a human listening verdict; if phonikud-tts wins, t11 cannot ship it without the deferred q4 licence decision and needs a /deviate
- [follow_up] q4 deferred by the operator: whether a non-commercial phonikud-tts may appear in Apache-2.0 fleet templates is decided after the A/B (frame v6)
- [unknown_nonblocking] h25 says the branch touches nothing under lobes/gateway/, but an honest stt/tts advert (c27) may need the gateway to pass new env to roles.py; if so one of the two confirmed conditions must give and the operator decides which
- [unknown_nonblocking] The tool-wait deadline has no natural default and what the user hears while waiting is undesigned (frame v2); t6 ships it as a knob, the default value is chosen from t17's observations
- [follow_up] First-audio latency is unbudgeted: generate and TTS are whole-reply stages inside a turn (frame v1). Sentence-level streaming of generate->phonikud->TTS is NOT in this plan; t17's timings decide whether it becomes the next plan
- [unknown_nonblocking] associate is a mesh-shared lane on the Orin (max_num_seqs=2); a voice turn behind a long-context prefill has no priority and no local fallback (frame v4)
