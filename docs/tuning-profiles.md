...
lobes resolves the vLLM serve config from **three layers**, so the same
catalog model can be served in different *gears* without editing the compose
file. `lobes switch <model> --purpose <p> --machine <m>` writes the resolved
...
3. **model** (the [catalog](../lobes/catalog.py)) — quantization, tool
   parser, and MoE-only serve extras
4. **explicit CLI flags** (`--max-model-len`, `--gpu-mem-util`, …) — always win

The tables live in [`lobes/profiles.py`](../lobes/profiles.py) (a pure
data module, guarded by `tests/test_profiles.py`). See `lobes explain tuning`.

## Workload profiles (`--purpose`)

The `purpose` is the request mix the deployment is tuned for. The default is
`balanced`. `lobes benchmark` defaults its `(input, output)` shape to the
...
optimum. Confirm them with `lobes benchmark --purpose <p>`.
...
it toward `0.85` with `lobes switch --gpu-mem-util 0.85`.
...
checkpoint, concurrency 16, with MTP) — **not** lobes's own measurements.
...
MoE = much faster decode — replicates. Re-run with `lobes benchmark --purpose <p>`
