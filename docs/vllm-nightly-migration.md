...
- `culture.yaml:8` declares the lobes agent's runtime backend as
  `model: vllm-local/unsloth/Qwen3.8-27B-NVFP4` — every response
  the `lobes` agent posts to the Culture mesh is generated by the exact 27B
  primary this doc benchmarks (the id here was `unsloth/Qwen3.6-27B-NVFP4`
  at the time this section was written; the qwen3.8-cortex-upgrade plan
  flipped it 2026-08-19, see §9 below). `docs/gateway-fleet.md:400-401`
  confirms the resolution path: "`culture.yaml`'s `model:
  vllm-local/unsloth/Qwen3.8-27B-NVFP4` resolves through the gateway on
  `:8000` as the default."
...
## 9. Digest bump for the Qwen3.8 cortex upgrade (2026-08-19)

**The nightly digest §1–§8 measured against is no longer the one shipped
today.** The qwen3.8-cortex-upgrade plan (t5) bumped the shared vLLM nightly
image every default-on generate/embed/rerank/hand/worker service pulls, and
flipped `PRIMARY_MODEL` from `unsloth/Qwen3.6-27B-NVFP4` to
`unsloth/Qwen3.8-27B-NVFP4` in the same change:

| | Before (this doc's §1–§8) | Now (current pin) |
|---|---|---|
| Digest | `sha256:7c5a10e9a8b3c8642f4d0463a41215176c0dd834b4f0967287c7e3e517cf1be9` | `sha256:8bd082c274fae025b7079498fe1da65182ba1d4c2188c0f5a68c1042c38c3695` |
| vLLM version | `0.23.1rc1.dev672` | `0.26.1rc1.dev942+g5a4c8d992` |
| Primary checkpoint | `unsloth/Qwen3.6-27B-NVFP4` | `unsloth/Qwen3.8-27B-NVFP4` |

The engine-unification story §4–§8 established (one nightly image across the
default-on fleet) is unchanged — only the pinned digest and the primary
checkpoint moved forward together, verified live against the official
nightly image on the GB10
(`docs/evidence/2026-08-19-spike-qwen3.8-official-nightly-spark.txt`). See
`docs/qwen3.8-27b-nvfp4.md` for the new primary's per-model doc and
`docs/specs/2026-08-19-qwen3-8-cortex-upgrade.md` /
`docs/plans/2026-08-19-qwen3-8-cortex-upgrade.md` for the upgrade's spec/plan.
