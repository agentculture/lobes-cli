...
- `COMPOSE_PROFILES`=llamacpp ALONE IS NOT ENOUGH -- it un-gates the service but does not route to it. The gateway defaults to `PRIMARY_URL`=${`PRIMARY_URL`:-<http://vllm-primary:8000}> (fleet/docker-compose.yml:1715), so a deployment that merely sets `LLAMACPP_`\* + `COMPOSE_PROFILES` would start vllm-primary AND probe it, silently testing the incumbent Qwen3.8-27B NVFP4 instead of Flash-Next. Routing is derived from the composed cortex model being a llama.cpp CATALOG GEAR (`shape_render.py`:189, `LLAMA_CPP_ACTIVATION_ENV` maps cortex -> llamacpp-primary and renders `PRIMARY_URL` + `PRIMARY_SERVED_NAME`). The separate deployment must therefore be rendered from a profile/shape whose cortex gear is the llama.cpp entry -- not hand-set env keys -- and vllm-primary must be absent from it, not merely unused.
  - instruction: Render the separate deployment with 'lobes init --shape <llamacpp-cortex shape> --apply'; confirm `PRIMARY_URL`=<http://llamacpp-primary:8000> in the rendered .env and that docker ps shows NO vllm-primary container before probing.
  - honesty: before any probe, the rendered .env shows `PRIMARY_URL` pointing at llamacpp-primary and `PRIMARY_SERVED_NAME` matching the GGUF gear, and docker ps confirms vllm-primary is not running -- a passing probe against a gateway still pointed at vLLM would invalidate the whole experiment
...
- `s37` — `qodo PR#218 review comment 2 vs docker-compose.yml:1715 (PRIMARY_URL default) and shape_render.py:189 (LLAMA_CPP_ACTIVATION_ENV)`: verified: `PRIMARY_URL` defaults to <http://vllm-primary:8000> and the llama.cpp routing is derived from the catalog gear via a shape, not from `COMPOSE_PROFILES`. The earlier claim c45 was wrong and would have produced a probe against the wrong model -- a false GO.
