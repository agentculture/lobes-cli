...
- **third opt-in-core-role shape** (`orin-associate`, lightning-on-orin plan,
  t9, now rendered and ACCEPTED live at its native 1M window — orin-associate-at-1m
  plan, t5) — the Orin's THIRD answer to the same `[cortex, senses, associate]`
  either/or/or: it hosts the opt-in `associate` lobe (Nemotron 3.5 Lightning
  30B-A3B; MEASURED live 2026-09-13 at `gpu_mem_util=0.70` /
  `max_model_len=1048576`, `docs/evidence/2026-09-13-measure-associate-budget-orin-1m.txt`,
  ACCEPTED on the rendered shape `docs/evidence/2026-09-13-accept-orin-associate-1m.txt`
  — HISTORY: an earlier 2026-08-25 measurement at `gpu_mem_util=0.63` /
  `max_model_len=128000` is superseded, below) with the FULL declaration in
  its own `[overrides.associate]`, and drops BOTH `cortex` and `senses` —
  plus, like its two sm_87 siblings, no `stt`/`tts`. See its row in the
  support table.
...
| **orin-associate** | `associate`, `embedder`, `reranker` — drops BOTH `cortex` and `senses`, hosts the opt-in `associate` lobe instead; `hand` is NOT hosted at the 1M budget (below); no `stt`/`tts` (same sm_87 reason as `orin-lobe`/`orin-cortex`) | **validated live at the native 1M window** | Pure data (`lobes/profiles/builtin_shapes/orin-associate.toml`), rendered and ACCEPTED live 2026-09-13. The Orin's THIRD answer to the `[cortex, senses, associate]` co-residency group (lightning-on-orin plan, t9; brought to 1M by the orin-associate-at-1m plan, t5): hosts the tenth Colleague role — `nvidia/NVIDIA-Nemotron-3.5-Lightning-30B-A3B-NVFP4` — with the FULL declaration in its own `[overrides.associate]` (approved deviation d1: the `orin` card marks `[roles.associate] feasible = false` and keeps the numbers as documentation only, mirroring how `thor-muse`/`thor-worker` carry their opt-in lobe's declaration). The checkpoint's own `hf_quant_config.json` is W4A16_NVFP4 (weight-only) on the experts plus FP8 elsewhere — NOT the W4A4 activation quantization that rules out `cortex`/`muse`'s NVFP4 exports on sm_87 — and vLLM v0.27.1 confirmed a full Marlin fallback stack live (`docs/evidence/2026-08-25-spike-lightning-vllm-orin.txt`: known-answer PASS, tool calls PASS, ~78-81 tok/s). Budget MEASURED 2026-09-13 (`docs/evidence/2026-09-13-measure-associate-budget-orin-1m.txt`, an NVFP4-vs-W4A16 A/B; NVFP4 kept — W4A16 missed the operator's >= 10% win rule): `gpu_mem_util=0.70`, `max_model_len=1048576` (the full native window), KV pool 2,889,456 tokens / 2.76x capacity ceiling at 1M, cold 1,040,073-token needle PASS (TTFT 2390.17 s, decode 7.98 tok/s at depth), minimum available host memory 2,588 MiB. The RENDERED shape was then ACCEPTED live (`docs/evidence/2026-09-13-accept-orin-associate-1m.txt`): `lobes init --shape orin-associate --profile orin --apply --force` booted associate at the same knobs, KV pool 2,899,067 tokens (2.76x), `GET /capabilities` context 128000 -> 1048576, all probes and a cold >= 1M needle PASS both non-streamed and streamed through the Orin gateway (proving `GATEWAY_READ_TIMEOUT=7200`), 2-session agentic PASS (24/24 tool calls, 6/6 recall), associate restarts 0, minimum available host memory **1,925 MiB** (approved deviation d2, issue #260: quote both this figure and the A/B's 2,588 MiB, never 2,588 MiB alone — the gap is not isolated). HISTORY (superseded 2026-09-13): an earlier 2026-08-25 measurement (`docs/evidence/2026-08-25-measure-associate-budget-orin.txt`) found `gpu_mem_util=0.63` (the vendor's own 0.70 was REFUSED at boot by 0.05 GiB) at `max_model_len=128000`, KV pool 1,249,280 tokens / 9.76x concurrency, with no acceptance transcript at the time. See [`docs/nemotron-3.5-lightning-30b-a3b-nvfp4.md`](nemotron-3.5-lightning-30b-a3b-nvfp4.md) and [`docs/orin-associate-deployment.md`](orin-associate-deployment.md). |
...
> **DECLARED/UNVALIDATED (#108), mesh-brain member only.** A mesh-provided
> role sources these same two surfaces from the roster instead of a
> `<PREFIX>_PEER_*` value: `hosted_by` is the verified peer's announced
> origin when the role has exactly one plain (agreeing-fingerprint) origin,
> or a `members` list in its place when the role is pooled across more than
> one; `GET /capabilities` `ready`/`proxied` come from that same verified
> roster. A role a mesh member has *announced but not yet probed* is a
> third state neither `404` names today: a request during that boot window
> answers `503`, `error.type`/`error.code` `role_unverified`,
> `error.hosted_by` naming the pending member, `Retry-After: 5`, never a
> `404` — see
