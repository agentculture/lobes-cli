...
- **The rest of the one-lobe-per-box end-state** (a Qwen-only Spark, a
  Gemma-only Thor) and the **cross-box referral** story (a box's gateway
  annotating an unhosted role with the peer that hosts it, rather than
  today's per-box-honesty-only contract) — tracked as issue #112, with its
  own spec + plan opened in PR #116.
