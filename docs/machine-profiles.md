...
### muse and worker: shape-declared, not card-declared
...
`worker` — see [`docs/deployment-shapes.md`](deployment-shapes.md) — while
...
[`docs/gemma-4-31b-nvfp4.md`](gemma-4-31b-nvfp4.md). `thor-worker`'s budget
...
| any card, `gateway-only` shape | any (card-agnostic) | any built-in card profile | **declared, UNVALIDATED (#108)** | The consumer-only mesh-brain member: `hosts = []`, no local generate/embed/rerank/audio lane at all, every role answered from the mesh-brain join's auto-wired proxying (see [`docs/gateway-fleet.md`](gateway-fleet.md#the-mesh-brain-join-opt-in-every-member-is-the-brain) and [`docs/deployment-shapes.md`](deployment-shapes.md)). Because it composes onto whatever card profile the box resolves and renders no role-specific overrides, its status is independent of which card boots it — no box, of any card, has booted it yet. |
