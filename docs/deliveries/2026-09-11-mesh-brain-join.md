...
| A gateway-only member boots, joins and serves every role by proxy | high | release run on 0.76.0: spark-gw joined, verified all three in 90 s, served cortex/worker/associate/embedder/reranker by proxy (`docs/evidence/2026-09-12-accept-mesh-brain-join-fleet.txt`, h8 section) |
| A robot client (Qwen Code) reaches every mesh role through its local gateway | high | release run on 0.76.0: `qwen -m worker` and `qwen -m associate` against the Spark gateway fixed the test file through the Thor and the Orin (h6 section of the transcript) |
