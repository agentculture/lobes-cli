...
transcript):** `context` for such a role is, since the review round, the serving peer's own advertised window captured by the same probe, and `model` (since 0.81.3) is the serving peer's own advertised model id, captured the same way. A pooled role publishes each one only when every named member agrees; otherwise the local value stays. on a
