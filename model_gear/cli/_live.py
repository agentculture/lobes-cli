...
``/metrics`` + ``/health`` via :mod:`model_gear._metrics` (best-effort — never
...
_FLEET_STATUS_OBJECT = "model-gear.fleet_status"
...
def live_sections(port: int, served_name: str | None) -> list[dict]:
    ...
    The gateway exposes ``/status`` (a model-gear fan-out); a bare vLLM does not, so
    ...
    return [
        {
            "title": "Live",
            "items": [
                f"no model-gear endpoint reachable on :{port}",
                ">> start one: model serve --apply  (or model fleet up --apply)",
            ],
        }
    ]
