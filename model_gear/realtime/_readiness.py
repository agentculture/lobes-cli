...
This module is **stdlib only** so it is importable and unit-testable in the
offline CI environment (no torch / nemo / fastapi installed there).  The heavy
container code that calls :func:`evaluate_readiness` lives in
``model_gear/templates/fleet/listen_server.py``.

Note: a vendored copy of this file lives at
``model_gear/templates/fleet/_readiness.py`` (the container build context).
Both copies must stay in sync — cite-don't-import convention.
"""
