...
def test_endpoints_for_by_task_family() -> None:
    ...
    assert "POST /v1/rerank" in eps
    assert "POST /v1/score" in eps
...
def test_fleet_status_payload_aggregates_busy_and_health() -> None:
    ...
    assert embed["health"] == "unreachable"
    assert embed["metrics"] is None
    primary = next(b for b in payload["backends"] if b["name"] == "primary")
    assert primary["task"] == "generate"
    assert primary["served_name"] == "P"
