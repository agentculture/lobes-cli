"""Boundary guard: lobes is SERVE-ONLY (issue #44).

lobes vectorizes and scores over HTTP — it is the serving half of the
...
2. No module inside lobes/ has a stem that names a storage or retrieval
...
# Module stems that belong to eidetic-cli, not lobes.
    ...
    for dep in all_deps:
        ...
        for marker in _VECTOR_DB_MARKERS:
            assert marker not in dep_lower, (
                ...
                f"belong in eidetic-cli, not lobes (issue #44)."
            )


def test_no_storage_or_retrieval_modules() -> None:
    """lobes/ must not contain any module named after a storage/retrieval concept."""
