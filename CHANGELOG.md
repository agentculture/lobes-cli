...
- `lobes-deploy` skill (local): the procedure for changing a LIVE box without confusing the packaged templates (`lobes/templates/**`) with the deployment (`~/.lobes`). Its `scripts/lobes-compose.sh` runs `docker compose` with the deployment's own `-f` chain (from `lobes fleet files`), refuses the template folder, and is dry-run until `--apply`. `.qwen/skills/lobes-deploy` links to it and a new `QWEN.md` points Qwen Code at it.
- `docs/operating-a-deployment.md`: templates vs deployment, hand-kept boxes, the one-service rebuild procedure, and the 2026-09-18 incident in which a compose run inside `lobes/templates/fleet/` replaced the Spark's ComfyUI container with one the gateway could not reach.
...
- Commit post-merge ledger state for plan orin-associate-at-1m: agex PR #261 event log and frame claim o16 (associate-first boot order, deviation d4); all four deviations d1-d4 are approved.
...
- Delivery closure for PR #254 (mesh boot window + mesh-sourced /capabilities): `docs/deliveries/2026-09-12-mesh-boot-window-and-capabilities-advert.md` (planned vs actual, deviations d1-d3, drift, evidence-backed claims, remaining work) and the devague ledgers it reads — evidence e1-e11 from the behavioral test runs on merged main and the live transcript, behavioral deltas b1-b5, lapses l1-l11 — all filed proposed for adjudication.
...
- Release-run evidence for the mesh-brain join on 0.76.0 from PyPI: signals 1-5 re-measured on all three boxes, Qwen Code through the mesh (h6) and a gateway-only fourth member (h8) both measured PASS; delivery summary and validate-delivery ledger updated (e11-e13).
