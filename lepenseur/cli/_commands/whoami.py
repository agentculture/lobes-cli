"""``lepenseur whoami`` — the smallest identity probe.
...
_FALLBACK_NICK = "lepenseur"


def _find_culture_yaml() -> Path | None:
    """Locate lepenseur's own ``culture.yaml`` by walking up from this module.

    The identity must be lepenseur's, not whatever ``culture.yaml`` happens to
...
def _read_agent_fields() -> dict[str, str]:
    ...
    Parsed without a YAML dependency to keep lepenseur's runtime deps empty.
...
def register(sub: argparse._SubParsersAction) -> None:
    p = sub.add_parser(
        "whoami",
        help="Report lepenseur's nick, version, backend, and served model.",
    )
