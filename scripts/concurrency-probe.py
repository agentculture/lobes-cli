...
MODEL = "worker"
N = int(sys.argv[1])
APPROX_PROMPT_TOKENS = int(sys.argv[2]) if len(sys.argv) > 2 else 4000
...
def make_prompt(seed):
    ...
    return (
        "Here is a source file. Summarise what it does in exactly one "
...
[t.join() for t in ths]
WALL = time.time() - T0
out = sum(r["out_tok"] for r in res)
pre = sum(r["prompt_tok"] for r in res)
ds = [r["decode_tok_s"] for r in res]
print(
    json.dumps(
        {
            "width": N,
            "prompts": "SHARED" if SHARED else "DISTINCT",
