"""``lobes whoami`` — the smallest identity probe.

Reports lobes' view of the world: the tool + version, the machine it runs
on, the *configured* served model (from the deployment ``.env`` — not a live
``/v1/models`` query; ``lobes overview --list`` shows the full supported catalog),
the container's health, and the agent that consumes the model (``lobes``, read
from ``culture.yaml``). Read-only.
"""
...
def register(sub: argparse._SubParsersAction) -> None:
    p = sub.add_parser(
        "whoami",
        help="Report the tool, machine, configured served model, container health, and agent.",
    )
