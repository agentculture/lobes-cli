...
``overview`` (see :mod:`lobes.cli._commands.cli`). Descriptive verbs never
...
from lobes.cli import _live, _runtime_ops
from lobes.cli._commands.whoami import report
...
_VERBS = [
    ...
    "capabilities [--json] — read-only: the eight first-class roles (cortex/senses/muse/"
    "worker/embedder/reranker/stt/tts) resolved to live endpoint + metadata (issue #81)",
...
def candidate_models() -> list[dict[str, str]]:
    ...
    Sourced from the packaged :mod:`lobes.catalog` so it is populated even in
...
def tool_sections(*, current: bool = False, listing: bool = False) -> list[dict[str, object]]:
    """Sections describing lobes the tool (used by the global verb)."""
    ident = report()
    if current:
...
def register(sub: argparse._SubParsersAction) -> None:
    p = sub.add_parser(
        "overview",
        help="Read-only snapshot of lobes (identity, verbs, served model, candidates).",
    )
    p.add_argument(
        "target",
        nargs="?",
        help="Ignored — overview always describes lobes itself.",
    )
    p.add_argument(
