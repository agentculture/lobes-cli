...
Every failure inside lobes raises :class:`ModelGearError`. The top-level
``main()`` catches it, formats via :mod:`lobes.cli._output`, and exits with
...
# Exit-code policy. Documented in ``lobes learn`` output.
# 0      = success
...
class ModelGearError(Exception):
    """Structured error raised within lobes; carries a remediation hint for agents."""
