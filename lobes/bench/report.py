...
def render_report(results: dict) -> str:
    ...
    # Helper to format a metric name for display
    ...
    def format_value(value) -> str:
        ...
        return (
            f"{value:.2f}".rstrip("0").rstrip(".") if isinstance(value, float) else str(int(value))
        )

    # Helper to format a delta
    def format_delta(minor_val, primary_val) -> str:
    ...
    for metric_key in ordered_metrics:
        ...
        minor_str = format_value(minor_val)
        primary_str = format_value(primary_val)
        delta_str = format_delta(minor_val, primary_val)
