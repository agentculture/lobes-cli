...
def _malformed(path: os.PathLike | str, problem: str) -> ModelGearError:
    """A uniform user error for a lock that is not shaped like a lock.

    A lock is adopted from a repository, so it is UNTRUSTED input: every way it
    can be wrong has to surface as a controlled error naming the file and the
    problem, never as a traceback out of ``lobes init --from-lock`` or ``lobes
    doctor``.
    """
    return ModelGearError(
        code=EXIT_USER_ERROR,
        message=f"{path}: malformed {LOCK_FILENAME} — {problem}",
        remediation=(
            f"a {LOCK_FILENAME} is generated by lobes, never hand-written: it carries "
            "`schema_version`, a [variation] table of strings, and string-valued [env] "
            "and [files] tables. Re-capture the variation's lock rather than editing it."
        ),
    )


def _lock_table(raw: Mapping[str, object], name: str, path: os.PathLike | str) -> Mapping:
    """``raw[name]`` as a table, or a controlled error."""
    value = raw.get(name, {})
    if not isinstance(value, dict):
        raise _malformed(path, f"[{name}] must be a table, not {type(value).__name__}")
    return value


def _lock_str_table(
    raw: Mapping[str, object], name: str, path: os.PathLike | str
) -> dict[str, str]:
    """``raw[name]`` as a table whose every value is a string."""
    table = _lock_table(raw, name, path)
    for key, value in table.items():
        if not isinstance(value, str):
            raise _malformed(
                path, f"[{name}] key {key!r} must be a string, not {type(value).__name__}"
            )
    return dict(table)


def _lock_str(
    table: Mapping, field: str, path: os.PathLike | str, *, default: str | None = None
) -> str | None:
    """``table[field]`` as a string (or *default* when absent)."""
    value = table.get(field, default)
    if value is not None and not isinstance(value, str):
        raise _malformed(path, f"[variation] {field} must be a string, not {type(value).__name__}")
    return value
...
def load_lock(path: os.PathLike | str) -> DeploymentLock:
    ...
    Every failure mode is a :class:`~lobes.cli._errors.ModelGearError`: an
    unreadable or non-UTF-8 file, TOML that does not parse, an unknown
    ``schema_version``, and any table or value that is not the type the schema
    declares. Nothing about the file's contents can raise a bare
    ``TypeError``/``AttributeError`` out of this function.
    """
    try:
        text = Path(path).read_text(encoding="utf-8")
    except UnicodeDecodeError as exc:
        raise _malformed(path, f"not UTF-8 text ({exc})") from exc
    except OSError as exc:
        raise ModelGearError(
            code=EXIT_USER_ERROR,
            message=f"{path}: cannot read {LOCK_FILENAME} ({exc.strerror})",
            remediation=f"pass a readable variation directory or {LOCK_FILENAME} path",
        ) from exc
    try:
    ...
    variation = _lock_table(raw, "variation", path)
    return DeploymentLock(
        variation=_lock_str(variation, "id", path, default="") or "",
        env=_lock_str_table(raw, "env", path),
        profile=_lock_str(variation, "profile", path),
        shape=_lock_str(variation, "shape", path),
        lobes_version=_lock_str(variation, "lobes_version", path),
        files=_lock_str_table(raw, "files", path),
        evidence=_lock_str(variation, "evidence", path),
    )
