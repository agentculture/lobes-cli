...
This is a VENDORED COPY of ``lobes/realtime/_readiness.py`` placed in the
Parakeet container build context (``lobes/templates/fleet/``).  The
Dockerfile COPYs it next to ``listen_server.py`` so the container can import it
as a top-level module without needing the ``lobes`` wheel.

IMPORTANT: keep this file in sync with the canonical copy at
``lobes/realtime/_readiness.py`` — cite-don't-import convention.
"""
